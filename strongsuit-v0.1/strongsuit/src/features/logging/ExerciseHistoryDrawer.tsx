...
function Sparkline({ data }: { data: number[] }) {
  ...
  return (
    <svg width={width} height={height} className="overflow-visible stroke-brand-500 fill-none" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
      <polyline points={pts} />
    </svg>
  )
}
...
export default function ExerciseHistoryDrawer({ clientId, exerciseId, open, onClose, clientUnits }: ExerciseHistoryDrawerProps) {
  ...
  if (!open) return (
    <dialog ref={dialogRef} className="m-0 h-full max-h-none w-full max-w-sm ml-auto bg-surface shadow-sheet backdrop:bg-iron-950/20 backdrop:backdrop-blur-sm open:animate-slide-left" onCancel={onClose}></dialog>
  )
  ...
  return (
    <dialog
      ref={dialogRef}
      onCancel={onClose}
      className="m-0 h-full max-h-none w-full max-w-sm ml-auto bg-surface shadow-sheet backdrop:bg-iron-950/20 backdrop:backdrop-blur-sm open:animate-slide-left p-0"
    >
      <div className="flex h-full flex-col">
        <div className="flex items-center justify-between border-b border-line px-4 py-4">
          <h2 className="text-lg font-bold text-ink">{exercise?.name || 'History'}</h2>
          <button onClick={onClose} className="p-2 -mr-2 text-faint hover:text-ink rounded-full hover:bg-iron-100 dark:hover:bg-iron-800 transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-8">
          {history.length > 0 && chartData.some(d => d > 0) && (
            <div className="flex items-center justify-between bg-brand-50/50 dark:bg-brand-950/20 rounded-lg p-4 border border-brand-500/20">
              <div>
                <div className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-brand-600 dark:text-brand-400 mb-1">
          ...
          {suggestion && (
            <div className="rounded-card border border-line bg-surface2 p-4">
              ...
              <div className="font-mono tnum text-xl font-semibold text-ink">
                ...
                <div className="mt-3 border-t border-line pt-2">
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1">
                    {warmupRamp(suggestion.load, clientUnits === 'kg' ? 1.25 : 2.5).map(s => (
                      <span key={s.pct} className="font-mono tnum text-xs text-muted">
            ...
            {history.length === 0 ? (
              ...
              history.map((h, i) => (
                <div key={i} className="relative pl-4 border-l-2 border-line">
                  <div className="absolute -left-1.5 top-1.5 h-2.5 w-2.5 rounded-full bg-iron-300 dark:bg-iron-700" />
